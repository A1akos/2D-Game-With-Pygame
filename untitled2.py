...
#karakter hareketlerini tanimlama
moving_left = False
moving_right = False
shoot = False
grenade = False
...
#arkaplan
cehennem_img = pygame.image.load('assets/img/Background/cehennem.png').convert_alpha()
...
def resim_arkaplan():
    ...
    for x in range(5):
        screen.blit(cehennem_img, ((x * width) - bg_scroll * 0.5, 0))
...
def reset_level():
    ...
    return data
#karakter classi
class asker(pygame.sprite.Sprite):
    def __init__(self, char_type, x, y, scale, speed, ammo, grenades):
        ...
        #bot degerleri
        self.hareket_counter = 0
        self.vision = pygame.Rect(0, 0, 150, 20)
        self.idling = False
        self.idling_counter = 0
        
        #oyuncu animasyonlari
    ...
    def update(self):
        self.animasyon()
        self.check_alive()
        #mermi cooldownu icin
        if self.shoot_cooldown > 0:
            self.shoot_cooldown -= 1


    def hareket(self, moving_left, moving_right):
    ...
    def ai(self):
        if self.alive and player.alive:
            if self.idling == False and random.randint(1, 200) == 1:
                self.aksiyon(0)#0: idle
                self.idling = True
                self.idling_counter = 50
            #botun oyuncuya yakin olup olmadigini kontrol etme
            if self.vision.colliderect(player.rect):
                ...
                if self.idling == False:
                    ...
                    self.hareket(ai_moving_left, ai_moving_right)
                    self.aksiyon(1)#1: run
                    self.hareket_counter += 1
                    #bot yurudukce olacak olan gorus guncellemesi
                    self.vision.center = (self.rect.centerx + 75 * self.direction, self.rect.centery)

                    if self.hareket_counter > TILE_SIZE:
                        self.direction *= -1
                        self.hareket_counter *= -1
                else:
    ...
    def animasyon(self):
        #animasyon guncellemesi
        ANIMATION_COOLDOWN = 100
        #dogru gorseli guncelleme
        self.image = self.animation_list[self.action][self.frame_index]
        #animasyon icin yeterli zamanin gectigini gosteren fonk
        if pygame.time.get_ticks() - self.update_time > ANIMATION_COOLDOWN:
            self.update_time = pygame.time.get_ticks()
            self.frame_index += 1
        #animasyonu loopa alma kismi
            ...
            for x, tile in enumerate(row):
                if tile >= 0:
                    ...
                    if tile >= 0 and tile <= 8:
                        ...
                        player = asker('player', x * TILE_SIZE, y * TILE_SIZE, 1.65, 5, 20, 5)
                        health_bar = HealthBar(10, 10, player.health, player.health)
                    elif tile == 16:#create enemies
                        enemy = asker('enemy', x * TILE_SIZE, y * TILE_SIZE, 1.65, 2, 20, 0)
...
class Bullet(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        #merminin ekrandan cikip cikmadigini kontrol etme
        if self.rect.right < 0 or self.rect.left > SCREEN_WIDTH:
            self.kill()
        #merminin duvara carpmasi
        for tile in world.obstacle_list:
...
while run:
    ...
    if start_game == False:
        ...
        if player.alive:
            ...
            #bomba atma
                ...
                grenade_thrown = True
            if player.in_air:
                player.aksiyon(2)#2: ziplama
            elif moving_left or moving_right:
                player.aksiyon(1)#1: kosma
            else:
                player.aksiyon(0)#0: idle
            screen_scroll, level_complete = player.hareket(moving_left, moving_right)
    ...
    for event in pygame.event.get():
        ...
        #klavyedeki tuslari serbest birakma
